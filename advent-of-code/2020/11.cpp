...
struct Dir {
	...
	bool in_bounds(int y, int min_y, int max_y, int x, int min_x, int max_x) const {
		auto [yy, xx] = this->apply(y, x);
	...
	pair<int, int> apply(int y, int x) const {
		int yy = y + dy;
		int xx = x + dx;
		return { yy, xx };
	}
...
EpochResult epoch(const vector<string>& seats) {
	...
	for (int y = 0; y < row_height; y++) {
		for (int x = 0; x < col_width; x++) {
			...
			auto count_seats = [&](Seat which_seat_to_count) -> int {
				int count = 0;

				for (const Dir& d : dirs) {
					if (!d.in_bounds(y, 0, row_height, x, 0, col_width)) {
						continue;
					}

					auto [yy, xx] = d.apply(y, x);
			...
			auto update_seat = [&](const char& seat) -> char {
				...
				} else if (seat == Seat::occupied && occupied_around_me >= 4) {
					return Seat::empty;
				} else {
					return seat;
				}
