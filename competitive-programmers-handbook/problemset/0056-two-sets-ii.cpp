...
ll solve(vector<vector<bool>>& ready, vector<vector<ll>>& value, ll begin, const ll n, ll halfsum) {
	...
	for (ll i = begin; i <= n; i++) {
		ll ret = solve(ready, value, i + 1, n, halfsum - i);
		sum += ret;
		sum %= INFx2;
...
int main() {
	...
	const ll mxA = (sum + 1) * 10;
	const ll mxB = (n + 1) * 10;
	vector<vector<bool>> ready(mxA, vector<bool>(mxB));
	vector<vector<ll>>   value(mxA, vector<ll>  (mxB));

	ll sol = solve(ready, value, 1, n, half);
